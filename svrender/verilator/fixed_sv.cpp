...
int main(int argc, char *argv[]) {
    ...
    dut.open_trace("simout/fixed_sv.fst");
    ...
    for (size_t T = 0; T < total_tests; T ++) {
        int32_t a0 = dist(rng),a1 = dist(rng),a2 = dist(rng), b = dist(rng);
        dut->a[0] = a0;
        dut->a[1] = a1;
        dut->a[2] = a2;
        dut->b = b;
        dut.tick();
        int32_t a01 = dist(rng),a11 = dist(rng),a21 = dist(rng), b1 = dist(rng);
        dut->a[0] = a01;
        dut->a[1] = a11;
        dut->a[2] = a21;
        dut->b = b1;
        dut.tick();
        int32_t a02 = dist(rng),a12 = dist(rng),a22 = dist(rng), b2 = dist(rng);
        dut->a[0] = a02;
        dut->a[1] = a12;
        dut->a[2] = a22;
        dut->b = b2;
        double result0 = to_fixed(int32_t(dut->res[0] << 5) >> 5);
        double result1 = to_fixed(int32_t(dut->res[1] << 5) >> 5);
        double result2 = to_fixed(int32_t(dut->res[2] << 5) >> 5);
        double ref0 = to_fixed(a0) * to_fixed(b);
        double ref1 = to_fixed(a1) * to_fixed(b);
        double ref2 = to_fixed(a2) * to_fixed(b);
