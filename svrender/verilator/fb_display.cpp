...
#include "Vfb_display.h"
#include "verilated_fst_c.h"

using std::chrono::nanoseconds;

using Module = Vfb_display;
...
int main(int argc, char *argv[]) {
    ...
    dut.open_trace("simout/fb_display.fst");

    for (size_t i = 0; i < 2; i ++)
    for (size_t x = 0; x < 1056; x ++)
    for (size_t y = 0; y < 625; y ++) {
        dut->hdata = x;
        dut->vdata = y;
        dut->sram_data = 0x01aa01bb;
        dut.tick();
    }
}
